...
class ListManageTestCase(TestCase):#检测列表管理功能
    ...
    def test_mem_subscribe(self):
        ...
        self.assertEqual(SubscribeList.objects.get(group = UserGroup.objects.get(group_name = 'all', user = self.user1)).mem, SubscribeMember.objects.get(mid = 400183186))
        self.assertEqual(SubscribeList.objects.get(group = UserGroup.objects.get(group_name = 'my_fav', user = self.user1)).mem, SubscribeMember.objects.get(mid = 400183186))
    ...
    def test_group_func(self):
        ...
        self.assertNotEquals(list(UserGroup.objects.filter(group_name = 'new_group', user = self.user1).values_list()),[])#测试是否已经增加新分组
        response = self.c.post('/subscribe/updategroup/',{'type':'rename','gid':UserGroup.objects.get(group_name = 'new_group').gid, 'group_name':'new_new_group'})
        self.assertEqual(response.json()['result'], 'success')#测试重命名分组的功能
        self.assertEquals(list(UserGroup.objects.filter(group_name = 'new_group', user = self.user1).values_list()),[])#测试原名字分组是否还存在
        self.assertNotEquals(list(UserGroup.objects.filter(group_name = 'new_new_group', user = self.user1).values_list()),[])#测试分组是否更新名字
        response = self.c.post('/subscribe/updategroup/',{'type':'rename','gid':UserGroup.objects.get(group_name = 'all', user =self.user2).gid, 'group_name':'new_new_group'})
        self.assertEqual(response.json()['result'], 'fail')#测试不能修改其他人的分组
        response = self.c.post('/subscribe/updategroup/',{'type':'delete','gid':UserGroup.objects.get(group_name = 'new_new_group').gid})
        self.assertEquals(list(UserGroup.objects.filter(group_name = 'new_new_group', user = self.user1).values_list()),[])#测试新分组是否已经被删除
    def test_mem_move(self):
        ...
        self.assertEqual(response.json()['result'], 'success')#测试关注是否成功                                                                                        
        self.assertEquals(list(SubscribeList.objects.filter(group = f1, mem = self.mem1).values_list('mem__mid',flat=True)),[self.mem1.mid])#测试是否关注成功
        self.assertEquals(list(SubscribeList.objects.filter(group = f2, mem = self.mem1).values_list('mem__mid',flat=True)),[self.mem1.mid])#测试是否关注成功
        ...
        self.assertEquals(list(SubscribeList.objects.filter(group = f1, mem = self.mem1).values_list('mem__mid',flat=True)),[self.mem1.mid])#测试是否移动分组成功
        self.assertEquals(list(SubscribeList.objects.filter(group = f2, mem = self.mem1).values_list()),[])#测试是否移动分组成功
        self.assertEquals(list(SubscribeList.objects.filter(group__user = self.user1, group__group_name = 'all', mem = self.mem1).values_list('mem__mid',flat=True)),[self.mem1.mid])#测试是否移动分组成功
        response = self.c.post('/subscribe/membermove/',{'mid':self.mem1.mid,'gid':[]})
        self.assertEquals(list(SubscribeList.objects.filter(group = f, mem = self.mem1).values_list('mem__mid',flat=True)),[self.mem1.mid])#测试不能修改其他人对用户的分组
    def test_show_list(self):
        ...
        SubscribeList.objects.create(mem = self.mem3,group = UserGroup.objects.get(user = self.user1, group_name = 'all'))#设置关注
        response = self.c.get('/subscribe/showlist/',{'gid':f1.gid})
        self.assertEquals(list(map(lambda d:d['mid'], response.json()['member_data'])), [self.mem1.mid,self.mem3.mid])#测试访问分组f1分组成员正确
        self.assertEquals(list(map(lambda d:d['group_name'], response.json()['group_list'])),['all','my_fav1','my_fav2'])#测试访问分组f1分组列表正确
